...
class Troop:
    ...
    def __post_init__(self) -> None:
        ...
        if self.path is None:
            self.path = []
        self.id = id(self)
        self.kind = max(0, min(self.kind, len(TROOP_KINDS) - 1))
        cap = TROOP_KINDS[self.kind].max_health
        if self.health > cap:
            self.health = cap
...
class City:
    ...
    def __post_init__(self) -> None:
        ...
        self.id = id(self)
        self.path = []
        self.production_kind = max(0, min(self.production_kind, len(TROOP_KINDS) - 1))
...
class Environment:
    ...
    def __init__(self) -> None:
        ...
        self.troop_vision_brushes = tuple(
            Brush(k.vision_radius, 1.0) for k in TROOP_KINDS
        )
    ...
    def _update_troop_health(
        ...
        cap = TROOP_KINDS[troop.kind].max_health
        if troop.health > cap:
            troop.health = cap
    ...
    def _move_troop_to_target(
        ...
        terrain_speed = on_terrain.speed_mod * TROOP_KINDS[troop.kind].speed_mult
        ...
        step = terrain_speed * TERRAIN_SPEED_MOD
        if distance < (step * 2):
            troop.path.pop(0)
    ...
    def _apply_combat(
        ...
        """
        if enemies_in_range:
            atk = TROOP_KINDS[troop.kind].attack_mult
            attack_power = on_terrain.attack_mod / 25 * atk
    ...
    def _update_troop_vision_and_border(
        ...
        if on_terrain is HILL:
            ...
            self.troop_vision_brushes[troop.kind].apply(
                player.vision, troop.position, 0
            )
    ...
    def update_cities(self, paths_to_apply: list) -> None:
        ...
        for i, city in enumerate(self.cities):
            ...
            if city.owner is not None:
                ...
                base_frames = SERVER_FPS * (
                    CITY_TROOP_GEN_RATE * max(1, t_per_c)
                ) * TROOP_KINDS[city.production_kind].production_time_mult
                if city.timer >= base_frames and t_per_c < CITY_TROOP_CAPACITY:
                    pk = city.production_kind
                    stats = TROOP_KINDS[pk]
...
class Game:
                ...
                while True:
                    ...
                    if player_in:
                        if player_in == "pause":
                            ...
                            if (
                                isinstance(player_in, list)
                                and len(player_in) > 2
                                and isinstance(player_in[2], list)
                            ):
    ...
    def game_logic(self) -> None:
        ...
        for p_num in range(world_info.players):
            for order in self.player_production_inputs[p_num]:
                try:
                    cid, kidx = int(order[0]), int(order[1])
                except (IndexError, TypeError, ValueError):
                    continue
                kidx = max(0, min(len(TROOP_KINDS) - 1, kidx))
                owner = self.environment.players[p_num]
                for c in self.environment.cities:
                    if c.id == cid and c.owner is owner:
                        c.production_kind = kidx
                        break
        self.player_production_inputs = [[] for i in range(world_info.players)]
