...
# Squared screen-pixel threshold: LMB on a city must move farther than this to start a rally path instead of opening the production menu.
CITY_DRAG_THRESHOLD_SQ = 10 * 10
...
class Game:
    ...
    def __init__(self, title: str) -> None:
        ...
        self.production_hovered_city_id = None
        self.production_button_rects: list[tuple[int, pygame.Rect]] = []
        self.production_menu_screen_pos: tuple[int, int] = (0, 0)
        self.prod_panel_height = 96

        self.selected_troop_ids: set[int] = set()
        self._city_press_start: tuple[int, int, int] | None = None
        self._city_press_world: tuple[float, float] | None = None
    ...
    def handle_mouse_down(self, e: pygame.event.Event) -> None:
        ...
        if e.button == 3 and not self.drawing_path and not self.drawing_city_path:
            ...
            self.handle_left_mouse_down(e)

    def handle_left_mouse_down(self, e: pygame.event.Event) -> None:
        """LMB: production popup buttons, troops (shift adds to selection), or city press (drag vs menu in motion/up)."""
        mx, my = e.pos[0], e.pos[1]
        shift = bool(e.mod & pygame.KMOD_SHIFT)

        kind_sel = self.production_kind_at_screen(mx, my)
        if kind_sel is not None and self.production_hovered_city_id is not None:
            if isinstance(self.player_input, list):
                self.player_input[2].append(
                    [self.production_hovered_city_id, kind_sel]
                )
            self.production_hovered_city_id = None
            return
        ...
        if best is not None:
            self._city_press_start = None
            self._city_press_world = None
            if shift:
                self.selected_troop_ids.add(best)
                return
            if best in self.selected_troop_ids and len(self.selected_troop_ids) > 1:
                to_path = set(self.selected_troop_ids)
            else:
                self.selected_troop_ids = {best}
                to_path = set(self.selected_troop_ids)
            self.start_paths_for_troop_ids(to_path)
            self.production_hovered_city_id = None
            return

        best_city = self.find_city_at_click(mx, my)
        if best_city is not None:
            self._city_press_start = (best_city, mx, my)
            self._city_press_world = (
                float(self.best_city_pos[0]),
                float(self.best_city_pos[1]),
            )
            return
        ...
        self._city_press_world = None
        if shift:
            self.selected_troop_ids.clear()
        elif self.production_hovered_city_id is not None:
            self.production_hovered_city_id = None

    def find_troop_at_click(self, mx: int, my: int) -> int | None:
        ...
        for row in troops:
            if len(row) >= 7:
                pos, tid, owner, path, health, attacking, _kind = row[:7]
            else:
                pos, tid, owner, path, health, attacking = row[:6]
    ...
    def find_city_at_click(self, mx: int, my: int) -> int:
        ...
        for row in cities:
            if len(row) >= 5:
                pos, cid, path, owner, _prod = row[:5]
            else:
                pos, cid, path, owner = row[:4]
            if owner == self.player_num:
                sx = int((pos[0] - self.camx) * self.zoom)
                sy = int((pos[1] - self.camy) * self.zoom)
                dx = mx - sx
    ...
    def get_troop_world_pos(self, tid: int) -> tuple[float, float] | None:
        """World position of troop `tid` from last draw_info, or None."""
        if self.draw_info is None:
            return None
        for row in self.draw_info[2]:
            if len(row) >= 7:
                pos, row_tid = row[0], row[1]
            else:
                pos, row_tid = row[0], row[1]
            if row_tid == tid:
    ...
    def start_paths_for_troop_ids(self, troop_ids: set[int]) -> None:
        """Begin drawing movement paths for each id; shared waypoints are appended from the first path's spacing."""
        self.paths = []
        self.drawing_path = False
        for tid in sorted(troop_ids):
            pos = self.get_troop_world_pos(tid)
            if pos is None:
                continue
            self.paths.append((tid, [pos]))
        if self.paths:
    ...
    def start_city_path(
        ...
        """Starts drawing a rally path for a city from `world_pos` (defaults to last hit-test)."""
        pos = world_pos if world_pos is not None else self.best_city_pos
        self.drawing_city_path = True
        self.remove_existing_city_path(cid)
        self.city_paths.append((cid, [tuple(pos)]))
    ...
    def handle_mouse_up(self, e: pygame.event.Event) -> None:
        ...
        if e.button == 3:
            self.panning = False
        if e.button == 1:
            if self._city_press_start is not None:
                cid, sx0, sy0 = self._city_press_start
                mx, my = e.pos
                if (mx - sx0) ** 2 + (my - sy0) ** 2 < CITY_DRAG_THRESHOLD_SQ:
                    self.production_hovered_city_id = cid
                    self.production_menu_screen_pos = (sx0, sy0)
                self._city_press_start = None
                self._city_press_world = None
            self.drawing_path = False
            self.drawing_city_path = False
    ...
    def handle_mouse_motion(self, e: pygame.event.Event) -> None:
        ...
        """
        if self._city_press_start is not None and e.buttons[0]:
            cid, sx0, sy0 = self._city_press_start
            mx, my = e.pos
            dx, dy = mx - sx0, my - sy0
            if dx * dx + dy * dy >= CITY_DRAG_THRESHOLD_SQ and self._city_press_world:
                wx, wy = self._city_press_world
                self.start_city_path(cid, mx, my, (wx, wy))
                self._city_press_start = None
                self._city_press_world = None
    ...
    def extend_troop_path(self, pos: tuple[int, int]) -> None:
        ...
        if not self.paths:
            return
        _, leader_pts = self.paths[0]
        lx, ly = leader_pts[-1]
        dx = wx - lx
        dy = wy - ly
        if dx * dx + dy * dy > (PATH_SPACING / max(1.0, self.zoom)):
            for _tid, p in self.paths:
                p.append((wx, wy))
    ...
    def handle_key_down(self, e: pygame.event.Event) -> None:
        ...
        if e.key == pygame.K_c:
            ...
            digit_to_kind = {pygame.K_1: 0, pygame.K_2: 1, pygame.K_3: 2}
            idx = digit_to_kind[e.key]
            if (
                isinstance(self.player_input, list)
                and self.production_hovered_city_id is not None
                and idx < len(TROOP_KINDS)
            ):
                self.player_input[2].append(
                    [self.production_hovered_city_id, idx]
                )
    ...
    def submit_paths(self) -> None:
        ...
        if not isinstance(self.player_input, list):
            return
    ...
    def draw(self) -> None:
        ...
        self._draw_production_panel()

    def _update_production_hover(self) -> None:
        """Track which owned city is under the cursor for the production bar."""
        if self.pause or self.draw_info is None:
            self.production_hovered_city_id = None
            return
        mx, my = pygame.mouse.get_pos()
        self.production_hovered_city_id = self.find_city_at_click(mx, my)

    def _city_production_kind(self, cid: int | None) -> int:
        """Return production_kind index for `cid`, or -1 if unknown."""
        if cid is None or self.draw_info is None:
            return -1
        for row in self.draw_info[3]:
            if len(row) >= 5 and row[1] == cid:
                return int(row[4])
            if len(row) == 4 and row[1] == cid:
                return 0
        return -1

    def production_kind_at_screen(self, mx: int, my: int) -> int | None:
        """If (mx, my) hits a production button, return its troop kind index."""
        for ki, rect in self.production_button_rects:
            if rect.collidepoint(mx, my):
                return ki
        return None

    def _draw_production_panel(self) -> None:
        """Bottom HUD: choose next unit type for the hovered friendly city."""
        self.production_button_rects.clear()
        if self.pause:
            return
        w, h = self.size
        strip_h = self.prod_panel_height
        overlay = pygame.Surface((w, strip_h), pygame.SRCALPHA)
        overlay.fill((28, 32, 40, 210))
        self.screen.blit(overlay, (0, h - strip_h))

        if self.production_hovered_city_id is not None:
            title = "Set this city's next trained unit (or press 1 / 2 / 3):"
        else:
            title = "Hover one of your cities to set production:"
        t_s = self.ui_font.render(title, True, (220, 225, 235))
        self.screen.blit(t_s, (16, h - strip_h + 6))

        bw, bh, gap = 152, 40, 10
        n = len(TROOP_KINDS)
        total_w = n * bw + (n - 1) * gap
        x0 = max(12, (w - total_w) // 2)
        y0 = h - strip_h + 38
        hover_ok = self.production_hovered_city_id is not None
        for i, tk in enumerate(TROOP_KINDS):
            rect = pygame.Rect(x0 + i * (bw + gap), y0, bw, bh)
            self.production_button_rects.append((i, rect))
            cur = self._city_production_kind(self.production_hovered_city_id)
            if hover_ok:
                bg = (55, 110, 75) if cur != i else (70, 130, 95)
            else:
                bg = (55, 60, 70)
            pygame.draw.rect(self.screen, bg, rect, border_radius=6)
            pygame.draw.rect(self.screen, (20, 24, 28), rect, 2, border_radius=6)
            lab = f"{i + 1}  {tk.name}"
            ls = self.ui_font.render(lab, True, (240, 245, 250))
    ...
    def draw_cities(self, cities: list, dynamic: pygame.Surface, z: float) -> None:
        ...
        Args:
            cities (list): List of city rows: (position, cid, path, owner[, production_kind]).
            dynamic (pygame.Surface): The dynamic surface to draw on.
            z (float): The zoom level.
        """
        for row in cities:
            if len(row) >= 5:
                position, cid, path, owner, prod_kind = row[:5]
            else:
                position, cid, path, owner = row[:4]
                prod_kind = 0
            ...
            if owner >= 0:
                ...
                if owner == self.player_num:
                    pk = max(0, min(len(TROOP_KINDS) - 1, int(prod_kind)))
                    abbr = TROOP_KINDS[pk].name[:2].upper()
                    px = int(position[0] * z)
                    py = int(position[1] * z)
                    ft = self.label_font.render(abbr, True, (35, 35, 35))
                    dynamic.blit(ft, (px - 6, int(py - 40 * z)))
    ...
    def draw_troops(self, troops: list, dynamic: pygame.Surface, z: float) -> None:
        ...
        Args:
            troops (list): List of troop rows: (position, tid, owner, path, health, attacking[, kind]).
            dynamic (pygame.Surface): The dynamic surface to draw on.
            z (float): The zoom level.
        """
        ...
        for row in troops:
            if len(row) >= 7:
                pos, tid, owner, path, health, attacking, kind = row[:7]
            else:
                pos, tid, owner, path, health, attacking = row[:6]
                kind = 0
            kind = max(0, min(len(TROOP_KINDS) - 1, int(kind)))
            stats = TROOP_KINDS[kind]
            max_hp = max(1.0, float(stats.max_health))
            off_x, off_y = (0, 0)
            if attacking:
            ...
            bar_frac = max(0.0, min(1.0, float(health) / max_hp))
            pygame.draw.rect(
                dynamic,
                (0, 255, 0),
                pygame.rect.Rect(
