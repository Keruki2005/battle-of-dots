...
class TroopKind:
    """Defines combat, movement, vision, and city production pacing for a trainable unit."""
    ...
    # Radius in world pixels for the server's vision brush (see TROOP_VISIBILITY_RADIUS baseline).
    vision_radius: float
    # Multiplies the city's base spawn interval; higher = longer time between units from that city.
    # Ordering (slowest first): Fast tank > Tank > Infantry > Radar infantry.
    production_time_mult: float


# Index order is wire protocol / production_kind (keep stable for saves/replays).
# Infantry: anchor numbers around legacy TROOP_HEALTH / default vision.
# Radar: glass cannon scout — big vision, fragile, slightly faster city production than infantry.
# Tank: slow bruiser — very slow production, chunky HP, low speed, slightly softer hits.
# Fast tank: mobile armor — still slowest production of all, solid HP, medium speed.
TROOP_KINDS: tuple[TroopKind, ...] = (
    TroopKind(
        "Infantry",
        100.0,
        1.0,
